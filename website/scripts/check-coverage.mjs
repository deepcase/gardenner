...
if (!docs.includes("运行时测试门禁（0.4.0 起）") || !docs.includes("npm run test:runtime") || !docs.includes("全部 66 种行为") || !docs.includes("双向比对真实实例成员")) errors.push("docs.html missing the runtime unit and DOM lifecycle test contract");
...
if (performance.regressions?.baselineVersion !== "0.9.0" || performance.regressions?.pass !== true || Object.keys(performance.regressions?.artifacts || {}).length !== 42) errors.push("Performance baseline regression report is missing, incomplete, or failing");
