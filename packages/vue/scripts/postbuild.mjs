...
await build({
  ...
  outfile: resolve(dist, "gardener-vue.min.js"),
  bundle: true,
  minify: true,
  format: "esm",
  platform: "browser",
...
await writeFile(resolve(dist, "catalog.json"), `${JSON.stringify({ schemaVersion: 1, version: "2.1.0", components: cssCatalog.components }, null, 2)}\n`);
...
await writeFile(resolve(dist, "gardener-vue.performance.json"), `${JSON.stringify({ $schema: "../metadata/performance.schema.json", schemaVersion: 1, version: "2.1.0", status: "passed", artifact: "gardener-vue.min.js", metrics }, null, 2)}\n`);
