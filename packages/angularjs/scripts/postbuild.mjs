...
await build({
  ...
  external: ["@gardener/css", "@gardener/css/*"], legalComments: "none",
});
const styles = {
...
await writeFile(resolve(dist, "gardener-angularjs.performance.json"), `${JSON.stringify({ $schema: "../metadata/performance.schema.json", schemaVersion: 1, version: "1.0.0", status: "passed", artifact: "gardener-angularjs.min.js", metrics }, null, 2)}\n`);
