...
const report = JSON.parse(await readFile(resolve(root, "dist/gardener-angularjs.performance.json"), "utf8"));
...
for (const metric of ["raw", "gzip", "brotli"]) if (actual[metric] > budget.limits[metric]) throw new Error(`${metric} budget exceeded: ${actual[metric]} > ${budget.limits[metric]}`);
