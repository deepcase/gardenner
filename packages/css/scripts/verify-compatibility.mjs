...
if (compatibility.baselineVersion !== "0.9.0") errors.push("1.0.0 compatibility baseline must preserve the complete 0.9.0 contract");
if (compatibility.policy.stage !== "stable") errors.push("1.0.0 compatibility policy must be stable");
...
for (const [group, baseline] of Object.entries(compatibility.baseline)) {
  ...
  if (compatibility.policy.stage === "stable" && compatibility.policy.stableTarget === pkg.version) {
    ...
    for (const value of available) if (!recorded.has(value)) errors.push(`${group}: 1.0.0 Stable baseline omitted an existing 0.9.0 contract (${value})`);
  }
}
