...
4. `npm run test:net11` 在安装 .NET 11 SDK 后，从 `artifacts` 中的 1.0.0 NuGet 包编译并运行独立 `net11.0` 消费者；它不允许回退为项目引用。

稳定包目标为 `net10.0`。这是 .NET 10/11 应用共同可消费的基础资产；`tests/Gardener.Blazor.Net11Consumer` 明确目标 `net11.0`，并验证 506 个组件、66 个行为、75 个事件和包版本，防止未来 SDK/API 变化破坏消费。
