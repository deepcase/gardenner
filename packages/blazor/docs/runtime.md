...
使用 `EventNames` 订阅任意 79 个 Gardenerim 事件，使用 `OnEvent` 接收事件名、detail 和取消状态。detail 中的 DOM 元素、File、BigInt、函数和循环引用会先转换为安全可序列化结构。对于 `beforeopen` 等同步守卫事件，将事件名放入 `PreventDefaultEvents`，浏览器会在跨越 JS/.NET 异步边界前立即取消默认动作。
