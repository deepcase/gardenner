...
const assemblyPath = resolve(root, "src", "Gardenerim.Blazor", "bin", "Release", "net10.0", "Gardenerim.Blazor.dll");
const assetPaths = await files(resolve(root, "src", "Gardenerim.Blazor", "wwwroot"));
const generatedPaths = await files(resolve(root, "src", "Gardenerim.Blazor", "Generated"));
const entrypointFiles = {
  css: resolve(root, "src", "Gardenerim.Blazor", "wwwroot", "gardener.min.css"),
  runtime: resolve(root, "src", "Gardenerim.Blazor", "wwwroot", "gardener.runtime.min.js"),
  blazor: resolve(root, "src", "Gardenerim.Blazor", "wwwroot", "gardener.blazor.js")
};
...
const packageName = nupkgs.find((name) => name === `Gardenerim.Blazor.${report.version}.nupkg`);
