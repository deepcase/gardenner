...
const generated = await readFile(resolve(root, "src", "Gardenerim.Blazor", "Generated", "GardenerimComponents.g.cs"), "utf8");
const catalog = await readFile(resolve(root, "src", "Gardenerim.Blazor", "Generated", "GardenerimCatalog.g.cs"), "utf8");
...
const project = await readFile(resolve(root, "src", "Gardenerim.Blazor", "Gardenerim.Blazor.csproj"), "utf8");
const consumer = await readFile(resolve(root, "tests", "Gardenerim.Blazor.Net11Consumer", "Gardenerim.Blazor.Net11Consumer.csproj"), "utf8");
...
assert(consumer.includes("<PackageReference Include=\"Gardenerim.Blazor\"") && !consumer.includes("ProjectReference"), "net11.0 must validate the packed NuGet rather than a source reference.");
...
assert((catalog.match(/public static readonly GardenerimComponentDefinition G/g) ?? []).length === 506, "Generated catalog does not contain 506 definitions.");
...
for (const asset of api.staticAssets) {
  const target = resolve(root, "src", "Gardenerim.Blazor", "wwwroot", asset);
...
for (const minified of ["gardener.min.css", "gardener.runtime.min.js", "gardener.tauri.min.js", "gardener.electron.min.js"]) {
  const content = await readFile(resolve(root, "src", "Gardenerim.Blazor", "wwwroot", minified), "utf8");
...
if (failures.length) {
  console.error(`Gardenerim.Blazor checks failed (${failures.length}):\n${failures.map((item) => `- ${item}`).join("\n")}`);
  process.exit(1);
}
console.log(`Gardenerim.Blazor API/schema/docs/assets checks passed: ${api.components} components, ${api.behaviors} behaviors, ${api.events} events, ${api.themeCount} themes, ${api.staticAssets.length} assets.`);
