...
if (cssApi.javascript.behaviors.length !== 72) throw new Error("Expected 72 Gardenerim behaviors");
if (cssApi.javascript.events.length !== 79) throw new Error("Expected 79 Gardenerim events");
...
/// <summary>Names of all 72 Gardenerim DOM behaviors.</summary>
...
const frameworkTypes = [
  ...
  "GardenerimJsModule", "GardenerimLocalizationOptions", "GardenerimLocalizationService", "GardenerimLocalizationState", "GardenerimOptions", "GardenerimPart", "GardenerimPlatform", "GardenerimProvider", "GardenerimRuntime", "GardenerimServiceCollectionExtensions", "GardenerimThemePresets",
...
const services = ["GardenerimRuntime", "GardenerimLocalizationService", "GardenerimThemeService", "GardenerimToastService", "GardenerimTauriService", "GardenerimElectronService"];
const staticAssets = ["gardener.core.min.css", "gardener.css", "gardener.min.css", "gardener.min.css.map", "gardener.runtime.min.js", "gardener.runtime.min.js.map", "locales.js", "gardener.blazor.js", "gardener.tauri.min.js", "gardener.tauri.min.js.map", "gardener.electron.min.js", "gardener.electron.min.js.map", ...["web", "mobile", "desktop", "tauri", "electron"].map((name) => `platforms/gardener.${name}.min.css`), ...builds.componentPacks.map(({ name }) => `components/${name}.min.css`)];
const publicApi = {
...
await writeFile(resolve(root, "metadata/components.json"), `${JSON.stringify({ $schema: "./components.schema.json", schemaVersion: 1, version: "2.1.0", count: definitions.length, components: definitions }, null, 2)}\n`);
...
await writeFile(resolve(root, "docs/behaviors.md"), `# Gardenerim DOM 行为完整目录\n\n共 ${cssApi.javascript.behaviors.length} 个行为，无省略。生成组件按元数据自动初始化；任意元素可使用 \`GardenerimBehavior\`；\`GardenerimRuntimeCatalog\` 提供机器可读成员契约。\n\n| 行为名 | 属性 | 实例成员 | C# 常量 |\n| --- | --- | --- | --- |\n${behaviorRows}\n`);
...
await writeFile(resolve(root, "docs/events.md"), `# Gardenerim 事件完整目录\n\n共 ${cssApi.javascript.events.length} 个 \`gardener:*\` 自定义事件，无省略。使用 \`EventNames\` 订阅、\`OnEvent\` 接收安全序列化 detail；守卫事件需要在 \`PreventDefaultEvents\` 中声明，以便浏览器同步执行 \`preventDefault()\`。\n\n| 事件名 | detail 键 | 守卫 | 冒泡 | 可取消 |\n| --- | --- | --- | --- | --- |\n${eventRows}\n`);
...
const copies = [
  ...
  ["dist/gardener.runtime.min.js", "gardener.runtime.min.js"], ["dist/gardener.runtime.min.js.map", "gardener.runtime.min.js.map"], ["dist/locales.js", "locales.js"],
