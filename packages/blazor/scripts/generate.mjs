...
const generatedRoot = resolve(root, "src/Gardener.Blazor/Generated");
const wwwroot = resolve(root, "src/Gardener.Blazor/wwwroot");
...
if (cssApi.javascript.behaviors.length !== 66) throw new Error("Expected 66 Gardener behaviors");
if (cssApi.javascript.events.length !== 75) throw new Error("Expected 75 Gardener events");
...
const componentClasses = definitions.map((item) => `/// <summary>Gardener ${item.name} component.</summary>\npublic sealed class ${item.componentType} : GardenerComponentBase\n{\n    protected override GardenerComponentDefinition Definition => GardenerCatalog.${item.componentType};\n}`).join("\n\n");
await writeFile(resolve(generatedRoot, "GardenerComponents.g.cs"), `// <auto-generated />\nusing Gardener.Blazor.Generated;\nusing Gardener.Blazor.Models;\n\nnamespace Gardener.Blazor.Components;\n\n${componentClasses}\n`);

const fields = definitions.map((item) => `    public static readonly GardenerComponentDefinition ${item.componentType} = ${definitionExpression(item)};`).join("\n");
const list = definitions.map((item) => `        ${item.componentType},`).join("\n");
await writeFile(resolve(generatedRoot, "GardenerCatalog.g.cs"), `// <auto-generated />\nusing Gardener.Blazor.Models;\n\nnamespace Gardener.Blazor.Generated;\n\n/// <summary>Complete generated catalog for all 506 Gardener Blazor components.</summary>\npublic static class GardenerCatalog\n{\n${fields}\n\n    public static IReadOnlyList<GardenerComponentDefinition> Components { get; } =\n    [\n${list}\n    ];\n\n    public static IReadOnlyDictionary<string, GardenerComponentDefinition> ByName { get; } = Components.ToDictionary(static component => component.Name, StringComparer.Ordinal);\n    public static IReadOnlyDictionary<string, GardenerComponentDefinition> ByComponentType { get; } = Components.ToDictionary(static component => component.ComponentType, StringComparer.Ordinal);\n}\n`);
...
await writeFile(resolve(generatedRoot, "GardenerThemePresets.g.cs"), `// <auto-generated />
namespace Gardener.Blazor.Generated;

/// <summary>Complete Gardener theme names and supported axis values.</summary>
public static class GardenerThemePresets
{
${themeConstants}
...
await writeFile(resolve(generatedRoot, "GardenerBehaviors.g.cs"), `// <auto-generated />
namespace Gardener.Blazor.Generated;

/// <summary>Names of all 66 Gardener DOM behaviors.</summary>
public static class GardenerBehaviors
{
${behaviorConstants}
...
await writeFile(resolve(generatedRoot, "GardenerEvents.g.cs"), `// <auto-generated />
namespace Gardener.Blazor.Generated;

/// <summary>Names of all 75 Gardener custom events.</summary>
...
await writeFile(resolve(generatedRoot, "GardenerRuntimeCatalog.g.cs"), `// <auto-generated />
using Gardener.Blazor.Models;

namespace Gardener.Blazor.Generated;

/// <summary>Complete behavior-member and custom-event contracts for runtime discovery and AI tooling.</summary>
public static class GardenerRuntimeCatalog
{
    public static IReadOnlyList<GardenerBehaviorDefinition> Behaviors { get; } =
    [
${behaviorDefinitions}
    ];
    public static IReadOnlyList<GardenerEventDefinition> Events { get; } =
    [
${eventDefinitions}
    ];
    public static IReadOnlyDictionary<string, GardenerBehaviorDefinition> BehaviorByName { get; } = Behaviors.ToDictionary(static item => item.Name, StringComparer.Ordinal);
    public static IReadOnlyDictionary<string, GardenerEventDefinition> EventByName { get; } = Events.ToDictionary(static item => item.Name, StringComparer.Ordinal);
}
`);
...
await writeFile(resolve(generatedRoot, "GardenerAssets.g.cs"), `// <auto-generated />
namespace Gardener.Blazor.Generated;

/// <summary>Static web asset paths for full, platform, and component-domain builds.</summary>
public static class GardenerAssets
{
    public const string BasePath = "_content/Gardener.Blazor/";
...
const frameworkTypes = [
  "GardenerAccessibilityDefinition", "GardenerAssets", "GardenerBehavior", "GardenerBehaviorDefinition", "GardenerBehaviors", "GardenerComponent", "GardenerComponentBase", "GardenerConstants", "GardenerElectronService", "GardenerEventArgs", "GardenerEventDefinition", "GardenerEvents", "GardenerField<TValue>",
  "GardenerJsModule", "GardenerOptions", "GardenerPart", "GardenerPlatform", "GardenerProvider", "GardenerRuntime", "GardenerServiceCollectionExtensions", "GardenerThemePresets",
  "GardenerRuntimeCatalog", "GardenerTauriService", "GardenerThemeService", "GardenerThemeState", "GardenerToastService", "GardenerValueChangedEventArgs", "GardenerComponentDefinition",
];
...
const services = ["GardenerRuntime", "GardenerThemeService", "GardenerToastService", "GardenerTauriService", "GardenerElectronService"];
const staticAssets = ["gardener.css", "gardener.min.css", "gardener.min.css.map", "gardener.runtime.min.js", "gardener.runtime.min.js.map", "gardener.blazor.js", "gardener.tauri.min.js", "gardener.tauri.min.js.map", "gardener.electron.min.js", "gardener.electron.min.js.map", ...["web", "mobile", "desktop", "tauri", "electron"].map((name) => `platforms/gardener.${name}.min.css`), ...builds.componentPacks.map(({ name }) => `components/${name}.min.css`)];
const publicApi = {
  ...
  packageId: "Gardener.Blazor", components: definitions.length,
...
await writeFile(resolve(root, "metadata/components.json"), `${JSON.stringify({ $schema: "./components.schema.json", schemaVersion: 1, version: "1.0.0", count: definitions.length, components: definitions }, null, 2)}\n`);
...
await writeFile(resolve(root, "docs/components.md"), `# Gardener Blazor 组件完整目录\n\n由 \`@gardener/css@1.0.0\` 元数据生成，共 506 个 Razor 组件，无省略。全部组件继承 \`GardenerComponentBase\`，共享 20 个参数与 6 个实例成员；状态与无障碍契约原样保留。\n\n| Razor 组件 | CSS 组件 | 分类 | 类型 | 状态 | 默认标签 | 根类/选择器 | 行为 | 无障碍契约 |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- |\n${componentRows}\n`);
const behaviorRows = cssApi.javascript.behaviorContracts.map((item) => `| \`${item.name}\` | \`${item.attribute}\` | ${item.instanceMembers.map((member) => `\`${member}\``).join(", ")} | \`GardenerBehaviors.${pascal(item.name).slice(1)}\` |`).join("\n");
await writeFile(resolve(root, "docs/behaviors.md"), `# Gardener DOM 行为完整目录\n\n共 66 个行为，无省略。生成组件按元数据自动初始化；任意元素可使用 \`GardenerBehavior\`；\`GardenerRuntimeCatalog\` 提供机器可读成员契约。\n\n| 行为名 | 属性 | 实例成员 | C# 常量 |\n| --- | --- | --- | --- |\n${behaviorRows}\n`);
...
await writeFile(resolve(root, "docs/events.md"), `# Gardener 事件完整目录\n\n共 75 个 \`gardener:*\` 自定义事件，无省略。使用 \`EventNames\` 订阅、\`OnEvent\` 接收安全序列化 detail；守卫事件需要在 \`PreventDefaultEvents\` 中声明，以便浏览器同步执行 \`preventDefault()\`。\n\n| 事件名 | detail 键 | 守卫 | 冒泡 | 可取消 |\n| --- | --- | --- | --- | --- |\n${eventRows}\n`);
const assetRows = staticAssets.map((name) => `| \`${name}\` | \`_content/Gardener.Blazor/${name}\` |`).join("\n");
await writeFile(resolve(root, "docs/assets.md"), `# 静态资源完整目录\n\n共 ${staticAssets.length} 个公开静态资源：全量 CSS、Blazor/runtime/桌面适配 JS、对应 source map、5 个平台包与 28 个组件域包，无省略。C# 应用也可通过 \`GardenerAssets\` 查询路径。\n\n| 包内路径 | 应用 URL |\n| --- | --- |\n${assetRows}\n`);
