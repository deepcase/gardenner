import * as Gardener from "./gardener.runtime.min.js";
...
export function initialize(element) {
  if (element) Gardener.init(element);
}

export function destroy(element) {
  if (element) Gardener.destroy(element);
}

export function refresh(element) {
  if (!element) return;
  Gardener.destroy(element);
  Gardener.init(element);
}
...
export function mount(element, shouldInitialize, valueEvent, valueKey, listenForValue, eventNames = [], preventDefaultEvents = [], receiver) {
  if (!element) return;
  unmount(element);
  if (shouldInitialize) Gardener.init(element); else Gardener.destroy(element);
  ...
  if (receiver) {
    for (const eventName of observed) {
      const listener = (event) => {
        ...
        if (listenForValue && eventName === valueEventName) receiver.invokeMethodAsync("NotifyGardenerValueChanged", serializable(normalizeValue(detailValue(detail, valueKey || "value"))), eventName, safeDetail).catch(() => {});
        if (subscribed.has(eventName)) receiver.invokeMethodAsync("NotifyGardenerEvent", safeDetail, eventName, event.defaultPrevented).catch(() => {});
      };
...
export function emit(element, name, detail) {
  return element ? Gardener.emit(element, String(name).replace(/^gardener:/, ""), detail ?? {}) : false;
}
...
export function getBehaviorMembers(element, behavior) {
  const instance = element ? Gardener.getInstance(element, behavior) : null;
  if (!instance) return [];
  const names = new Set();
...
export function invokeBehavior(element, behavior, member, args = []) {
  const instance = element ? Gardener.getInstance(element, behavior) : null;
  if (!instance) throw new Error(`Gardener behavior is not initialized: ${behavior}`);
  const target = instance[member];
  if (typeof target !== "function") throw new Error(`Unknown Gardener behavior member: ${behavior}.${member}`);
