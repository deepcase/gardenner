...
/// <summary>Shared rendering, value binding, and DOM lifecycle for every generated Gardener component.</summary>
public abstract class GardenerComponentBase : ComponentBase, IAsyncDisposable
{
    private DotNetObjectReference<GardenerComponentBase>? _receiver;
    private string? _mountedSignature;
    private bool _mounted;

    [Inject] private GardenerRuntime Runtime { get; set; } = default!;

    protected abstract GardenerComponentDefinition Definition { get; }
    ...
    [Parameter] public EventCallback<GardenerValueChangedEventArgs> OnValueChange { get; set; }
    ...
    [Parameter] public EventCallback<GardenerEventArgs> OnEvent { get; set; }
    ...
    [JSInvokable]
    public async Task NotifyGardenerValueChanged(JsonElement? value, string eventName, JsonElement? detail)
    {
        ...
        {
            await OnValueChange.InvokeAsync(new GardenerValueChangedEventArgs(normalized, eventName, detail?.Clone()));
        }
    }

    [JSInvokable]
    public async Task NotifyGardenerEvent(JsonElement? detail, string eventName, bool defaultPrevented)
    {
        if (OnEvent.HasDelegate)
        {
            await OnEvent.InvokeAsync(new GardenerEventArgs(detail is null ? null : NormalizeJson(detail.Value), eventName, defaultPrevented));
        }
    }
    ...
    private async Task HandleNativeValueAsync(ChangeEventArgs args)
    {
        ...
        if (OnValueChange.HasDelegate)
        {
            await OnValueChange.InvokeAsync(new GardenerValueChangedEventArgs(args.Value, "change"));
        }
    }

    private string BuildClass(GardenerComponentDefinition definition)
    {
